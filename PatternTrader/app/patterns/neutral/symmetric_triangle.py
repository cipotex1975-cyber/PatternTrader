...
logger = get_logger("SymmetricTrianglePattern")


@register_pattern
class SymmetricTrianglePattern(BasePattern):
    @property
    def name(self) -> str:
    ...
    def detect(
        ...
        width = line_at(res_slope, res_intercept, 0) - line_at(sup_slope, sup_intercept, 0)
        if width <= 0:
            return None
    ...
    def _calculate_confidence(
        ...
        convergence = min(1.0, max(0.0, abs(res_slope) / (res_end * 0.005)))
        symmetry = min(1.0, abs(abs(res_slope) - abs(sup_slope)) / (res_end * 0.005) + 0.3)
