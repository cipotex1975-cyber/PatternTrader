...
logger = get_logger("FallingWedgePattern")
...
class FallingWedgePattern(BasePattern):
    ...
    def _calculate_confidence(
        ...
        falling = min(1.0, max(0.0, -sup_slope / (sup_end * 0.005)))
        convergence = min(1.0, max(0.0, (sup_slope - res_slope) / (sup_end * 0.005)))
        ratio = min(1.0, height / sup_end * 20)

        confidence = 0.4
        confidence += falling * 0.2
