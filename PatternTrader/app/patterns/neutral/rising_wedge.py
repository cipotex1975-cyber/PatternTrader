...
logger = get_logger("RisingWedgePattern")


@register_pattern
class RisingWedgePattern(BasePattern):
    @property
    def name(self) -> str:
        return "rising_wedge"
    ...
    def _calculate_confidence(
        ...
        rising = min(1.0, max(0.0, res_slope / (res_end * 0.005)))
        convergence = min(1.0, max(0.0, (sup_slope - res_slope) / (res_end * 0.005)))
        ratio = min(1.0, height / res_end * 20)

        confidence = 0.4
        confidence += rising * 0.2
