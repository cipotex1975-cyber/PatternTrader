...
logger = get_logger("DescendingTrianglePattern")


@register_pattern
class DescendingTrianglePattern(BasePattern):
    @property
    def name(self) -> str:
    ...
    def detect(
        ...
        if abs(sup_slope) > 0.001 * sup_end:
            return None
        if res_slope >= -0.0001 * res_end:
            return None
        if sup_end >= res_end:
            return None

        height = highs.max() - sup_end
        if height / sup_end < 0.02:
            return None
        ...
        return PatternResult(
            ...
            confidence=self._calculate_confidence(res_slope, sup_slope, height, sup_end),
            key_levels={
                "neckline": sup_end,
                "resistance": line_at(res_slope, res_intercept, 0),
                "peak1": float(highs.max()),
                "apex": self._apex(res_slope, res_intercept, sup_slope, sup_intercept),
                "target": sup_end - height,
            },
    ...
    def _apex(
        ...
        return line_at(res_slope, res_intercept, x)

    def _calculate_confidence(
        self, res_slope: float, sup_slope: float, height: float, sup_end: float
    ) -> float:
        flatness = max(0.0, 1.0 - abs(sup_slope) / (sup_end * 0.01))
        convergence = min(1.0, max(0.0, -res_slope / (sup_end * 0.005)))
        ratio = min(1.0, height / sup_end * 20)
