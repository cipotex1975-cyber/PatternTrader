...
logger = get_logger("RectanglePattern")
...
class RectanglePattern(BasePattern):
    ...
    def detect(
        ...
        return PatternResult(
            ...
            confidence=self._calculate_confidence(height, res_end, res_slope, sup_slope),
            key_levels={
                "neckline": res_end,
                "resistance": res_end,
                "support": sup_end,
    ...
    def _calculate_confidence(
        self, height: float, res_end: float, res_slope: float, sup_slope: float
    ) -> float:
        flatness = max(
            0.0,
            1.0 - (abs(res_slope) + abs(sup_slope)) / (res_end * 0.01),
        )
        ratio = min(1.0, height / res_end * 20)
        parallel = min(1.0, abs(res_slope - sup_slope) / (res_end * 0.005) + 0.5)
